...
def plot_calibration_curves():
    cal_p1_ribose = np.array(
        [data.loc[well, "ribose_area_normalized"] for well in cond.CAL_P1_GLUCOSE]
    )

    cal_p2_ribose = np.array(
        [data.loc[well, "ribose_area_normalized"] for well in cond.CAL_P2_GLUCOSE]
    )
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(
    ...
    fig.update_layout(
        xaxis=dict(
            ...
            title="Glucose Concentration (mM)",
            range=[-4, 1],
            dtick=1,
        ),
        yaxis=dict(
    ...
    fig.write_image("plots/ribose_calibration_curves_p1.pdf")
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            name="Ribose",
            marker=dict(color=colors["Ribose"], line=dict(color="black", width=1.2)),
        )
    )
    ...
    fig = style_plot(fig, marker_size=10, line_thickness=3, font_size=14)
    fig.write_image("plots/ribose_calibration_curves_p2.pdf")
...
def plot_all_conditions(cond):
    ...
    fnames = [
        "ribose_at_succinate.svg",
        "ribose_at_glucose.svg",
        "ribose_oa_succinate.svg",
        "ribose_oa_glucose.svg",
        "ribose_atoa_succinate.svg",
        "ribose_atoa_glucose.svg",
        "ribose_at_succinate_glucose.svg",
        "ribose_atc2_succinate_glucose.svg",
        "ribose_oa_succinate_glucose.svg",
        "ribose_oac2_succinate_glucose.svg",
        "ribose_atoa_succinate_glucose.svg",
        "ribose_atoac2_succinate_glucose.svg",
    ]
    ...
    for wells, title, xs, ys, fname in zip(conds, titles, xs_list, ys_list, fnames):
        ...
        fig.update_layout(
            ...
            yaxis=dict(title="Area", type="log", exponentformat="power"),
            title=title,
            showlegend=False,
        )
